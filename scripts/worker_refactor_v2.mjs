...
const original = fs.readFileSync('src/app/api/render/temp.ts', 'utf8');
...
const bottomRegex = /\} catch \(err: unknown\) \{[\s\S]*?\} \/\/ end IIFE/i; // We don't need // end IIFE
...
fs.writeFileSync('src/app/api/render/worker/route.ts', fixed);
console.log('Worker replaced via Regex');

...
import { useSpySession, type CreativeBlueprint, type GeneratedJob } from "@/hooks/useSpySession";
...
export default function SpyProjectPage() {
        ...
        async function loadProject() {
            try {
                ...
                if (data.jobs.length > 0) {
                    data.jobs.forEach((j: GeneratedJob) => addJob(j));
                }

                // Determine step
                if (data.jobs.some((j: GeneratedJob) => j.status === "done")) {
                    setStep("results");
                } else if (data.blueprints.length > 0) {
                    setStep("creatives");
            ...
            for (const blueprint of selectedBlueprints) {
                ...
                try {
                    const res = await fetch("/api/spy/generate", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ blueprintId: blueprint.id, productImageUrl, resolution }),
                    });
                    const data = await res.json();
    ...
    const handleRegenerate = useCallback(
        async (blueprintId: string, imgUrl: string, resolution: string) => {
            ...
            if (!res.ok || data.error) {
                toast.error(
                    res.status === 402 ? t("errors.insufficientSparks") : t("errors.generationFailed"),
                );
            }
        },
        [addJob, t],
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            {/* Step 3: Creatives + product upload */}
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <label className="text-xs text-amber-400 hover:underline cursor-pointer">
                                        Change image
                                        <input type="file" accept="image/*" className="sr-only" onChange={handleProductImageUpload} />
                                    </label>
                                </div>
                            </div>
                        ) : (
